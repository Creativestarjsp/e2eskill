...
class CodeBrain:
    """Deterministic repository graph with a dependency-free fallback parser.

    If tree-sitter bindings are installed by a host runtime, they may be added as
    a future provider. The native MVP deliberately has no mandatory dependency.
    """
    ...
    def _extract(self, path: Path, text: str) -> tuple[list[dict], list[dict]]:
        ...
        for pattern, kind in patterns:
            for m in re.finditer(pattern, text):
                ...
                seen.add(key)
                line = text.count("\n", 0, m.start()) + 1
                symbols.append({"name": name, "kind": kind, "file": rel, "line": line, "id": f"{rel}:{name}:{line}"})
        for m in re.finditer(r"(?:from\s+['\"]([^'\"]+)['\"]|import\s+['\"]([^'\"]+)['\"]|import\s+([\w.]+))", text):
            target = next(x for x in m.groups() if x)
            edges.append({"type": "imports", "from": rel, "to": target})
        ...
        for name in local_names:
            for m in re.finditer(r"\b" + re.escape(name) + r"\s*\(", text):
                ...
                if owner and owner["name"] != name:
                    edges.append({"type": "calls", "from": owner["id"], "to": f"{name}", "file": rel, "line": line})
    ...
    def build(self) -> dict[str, Any]:
        ...
        for path in self._files():
            ...
            try:
                ...
                s, e = self._extract(path, text)
                symbols.extend(s)
                edges.extend(e)
            except Exception as exc:
                errors.append({"file": rel, "error": str(exc)})
        self.data = {"schema_version": SCHEMA_VERSION, "provider": "native-regex-mvp", "revision": _revision(self.root), "files": files, "symbols": symbols, "edges": edges, "errors": errors}
    ...
    def check(self) -> dict[str, Any]:
        ...
        return {"fresh": not changed and self.data.get("revision") == _revision(self.root), "changed": changed, "indexed_files": len(indexed)}
    ...
    def context(self, task: str) -> dict[str, Any]:
        ...
        return {"objective": task, "relevant_files": files[:20], "relevant_symbols": hits[:50], "tests": [p for p in files if "test" in p.lower() or "spec" in p.lower()], "provenance": {"provider": self.data.get("provider"), "revision": self.data.get("revision"), "coverage": "partial" if self.data.get("errors") else "structural"}}
